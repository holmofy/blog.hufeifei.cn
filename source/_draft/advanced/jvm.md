...
上图中reserved是操作系统保留的虚拟地址空间，在虚拟机刚运行时只会分配`-Xms`大小的物理内存，而且Java堆会通过以下策略尽可能的减少物理内存的消耗。
...
* 串行收集器使用单个线程来执行所有垃圾收集工作。因为线程之间没有通信开销，所以回收效率较高。它最适合单处理器机器，因为它无法利用多处理器硬件。它对于具有小数据集（最大约100 MB）的多处理器应用程序也非常有效。默认情况下，JVM会根据硬件和操作系统配置上选用串行收集器，或者可以使用`-XX:+UseSerialGC`选项显式启用串行收集器。

![串行GC](http://ww1.sinaimg.cn/large/bda5cd74ly1fxrar3t3u9j20ev04hglm.jpg)

* 并行收集器（也称为[吞吐量](https://translate.google.cn/#view=home&op=translate&sl=en&tl=zh-CN&text=throughput)收集器）并行执行MinorGC，这可以显着减少垃圾收集开销。它适用于在多处理器硬件上运行的具有中型到大型数据集的应用程序。默认情况下，JVM会根据硬件和操作系统配置上选用并行收集器，或者可以使用`-XX:+UseParallelGC`选项显式启用并行收集器。

  并行压缩是一种使收集器能够并行执行MajorGC的功能。如果没有并行压缩，MajorGC将使用单个线程执行，这可能会显着限制可伸缩性。如果`-XX:+UseParallelGC`已指定选项，则默认启用并行压缩。关闭它的选项是`-XX:-UseParallelOldGC`。
