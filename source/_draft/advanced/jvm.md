...
这篇文章的目的就是结合C/C++和Java，突破这座藩篱，尽量让更多的人理解JVM的垃圾回收机制。

# 1、GC之前

早在1960年，[Lisp语言](https://en.wikipedia.org/wiki/Lisp_%28programming_language%29)中就有自动垃圾收集的算法，只是那时候还没有形成[*Garbage Collection*](https://en.wikipedia.org/wiki/Garbage_collection_%28computer_science%29)这个概念。直到1996年，GC因为[Java](https://en.wikipedia.org/wiki/Java_%28programming_language%29)的出现而一举成名。在GC被广泛运用之前程序员们手动管理内存都会出现什么问题呢？
...
在堆上分配那就避免不了使用C++的`new`操作符。与`malloc`和`free`一样`new`也要和`delete`成对出现。但是在一个逻辑复杂的函数中难免会出现忘记释放内存就返回了：
...
但是应用的开发随着互联网发展的速度逐步提高要求。不能在指针引用上花费太多精力，也不能因为内存泄漏导致应用跑了个把礼拜就宕机了。
...
判断[垃圾对象](https://en.wikipedia.org/wiki/Garbage_%28computer_science%29)的方法有两种：[引用计数](https://en.wikipedia.org/wiki/Reference_counting)、[可达性分析](https://en.wikipedia.org/wiki/Unreachable_memory)
...
由于新生代对象朝生夕死，存活的对象数不多，使用复制算法时复制操作不会特别损耗性能。同时Eden区空间通常大于Suvivor区，而且Minor GC后，Eden区仍作为对象分配的地方，所以不像传统复制算法那样浪费一半的空间。
...
![HotSpot JVM Architecture](https://www.oracle.com/webfolder/technetwork/tutorials/obe/java/gc01/images/gcslides/Slide1.png)
...
运行时常量池都是从方法区中分配的。[当类或接口被创建](https://docs.oracle.com/javase/specs/jvms/se8/html/jvms-5.html#jvms-5.3)时常量池会被一起创建。
...
![Heapspot key Component](https://www.oracle.com/webfolder/technetwork/tutorials/obe/java/gc01/images/gcslides/Slide2.png)

其中JIT即时编译器除了负责将JVM指令转换成机器指令外，还会进行自适应的优化。比如：

1、为频繁调用的热点代码提供编译缓存提高运行时性能。

2、通过[逃逸分析](https://en.wikipedia.org/wiki/Escape_analysis)将原本需要在堆上分配的对象转换为栈分配，栈上分配对象的好处是随着函数返回，对象内存会自动回收，而不需要通过GC回收，间接地减少了GC的运行频率。

3、将调用频率高且体量小的方法进行内联，这和C++的内联函数作用一样。减少函数调用，可以减少因入栈消耗的时间。

JIT的内容不是本文的核心，有兴趣的可以自行谷歌。
...
`-XX:SurvivorRatio`可以设置新生代中Eden区域Survivor区的比例。

![survivorRatio](http://ww1.sinaimg.cn/large/bda5cd74ly1fxo9eyw4a9g20ki06gt8j.gif)

`-XX:MaxTenuringThreshold`可以设置新生代到老年代的老化年龄。
...
虽然永久代可以通过`-XX:PermSize`和`-XX:MaxPermSize`进行调节，但由于永久代没有很好的垃圾回收机制，偶尔会由于**PermGen**内存不足导致OutOfMemoryError。

所以Java8中用Metaspace取代了PermGen。

![Metaspace](http://ww1.sinaimg.cn/large/bda5cd74ly1fxo9oiczhbj20e908cwep.jpg)

新的[Metaspace支持四个参数](https://docs.oracle.com/javase/8/docs/technotes/guides/vm/gctuning/considerations.html)：

- *MetaspaceSize*和*MaxMetaspaceSize* - 可以设置Metaspace和最大内存大小。
- *MinMetaspaceFreeRatio -*  垃圾收集后可用的类元数据容量的最小百分比
- *MaxMetaspaceFreeRatio*  - 垃圾收集后可用的类元数据容量的最大百分比

*MetaspaceSize*是元数据空间的初始大小，当空间中类数据达到这个大小时，会触发
