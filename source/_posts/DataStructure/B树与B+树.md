...
2.每个中间节点(根节点和叶子节点除外)都包含k-1个元素和k个孩子节点，其中 m/2 <= k <= m
...
从时间复杂度上分析，不考虑IO问题，BTree阶数的升高并不会带来多少性能的提升。但B树就是为了减少IO而设计的，一般B树节点的大小符合读取磁盘块的大小即可。

从空间复杂度分析

节点中的数据是连续存储的(数组实现)，比如下面这样的简单定义
...
大多数节点中的数据并不会存满整个数组。这样导致**阶数越高造成的空间浪费也就越高**。当然磁盘上浪费一点也无所谓，拿低廉的磁盘空间换取性能的提升还是很值得的。
