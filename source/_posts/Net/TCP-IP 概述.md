...
1. **物理层**：主要功能是**利用物理传输介质为链路层提供基于比特流的数据传输**；同时制定物理接口的相关标准(如接口的机械特性、电气特性、规程特性等)，这些标准一般由OSI或IEEE等协会以及各大硬件厂商(通常也是协会成员)协同制定。比如有线网卡接口与双绞线接口线序，蓝牙或WiFi信号的频段，甚至USB接口、Type-C接口，ThunderBolt等都是他们制定的。
   ...
   >
   > 这样设计的初衷是：让终端设备的 TX 对接网络设备的 RX，实现通信。
   >
   > 现代设备大多不再需要交叉线: 从千兆以太网（1000BASE-T）开始，以及后来的万兆以太网，绝大多数设备都支持 Auto-MDIX（Automatic Medium-Dependent Interface Crossover） 功能：
   >
   > * 设备能自动检测线缆类型和对方接口类型。
   > * 自动切换内部 TX/RX 引脚，无论你用直通线还是交叉线，都能正常通信。
   > * 因此，在 2000 年代后期以后的电脑、交换机、路由器上，几乎全部支持 Auto-MDIX。
   > 
   >  所以现在：只用直通线就够了！
