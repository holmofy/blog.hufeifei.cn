...
## 用%w快捷方式声明数组，其中w就是word的缩写
## 实等价于'ann richard william susan pat'.split(' ')
...
m = /#{place}/.match("Go to 東京都")
    #=> #<MatchData "東京都">
puts m[0]  # 東京都
## 其他语言一样m[0]是整个正则匹配组，如果正则中有其他捕获组索引从1开始
```
...
```ruby
## 要尝试使用Symbol.all_symbols.include? :new_symbol的方式检测符号是否存在
## 为当这条语句解析时:new_symbol已经添加到符号表中了，执行的结果肯定是true
## 以这里要转成字符串
...
my_method(arg1, arg2)
my_method arg1, arg2
## 是同时调用两个方法发生歧义，Ruby会报语法错误
## 如下面的arg3不知道应该穿给method_one还是method_two
