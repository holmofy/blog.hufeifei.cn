...
possible_keys: idx_tb_test_c1_c2_c3_c4
     ...
     filtered: 100.00
        Extra: Using index
1 row in set, 1 warning (0.00 sec)
## 引的使用与where条件的顺序没有任何关系，解析器将SQL解析成树后，这几个条件的层级是一样的
...
## 面这个语句会用到c1,c2列进行精确匹配，c3,c4列用来分组
...
## 果group by的顺序与索引顺序不一致,group by子句仍会filesort并创建中间表
