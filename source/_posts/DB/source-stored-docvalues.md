...
如果禁用 elasticsearch `_source` 字段，更新文档时需要从头开始重新索引。实际上，为了更新文档，我们需要从旧文档中获取字段的值。从逻辑上讲，使用Stored Field和doc_values从旧文档中获取字段的值应该是可行的（这就是 Solr 中原子更新的工作方式）。但是，由于设计决定，这在 ElasticSearch 中是不允许的，如果您需要更新文档，则必须在 elasticsearch 索引配置中启用`_source`字段。
...
如果磁盘空间不是问题，**您甚至可以混合不同的方法并将字段设置为`store_field`和`doc_values`，并保持开启`_source`**。在查询时，elasticsearch 使您可以选择所需的字段列表，以及是否希望从 `_source`、`_store_field`或 `doc_values` 返回它们。
