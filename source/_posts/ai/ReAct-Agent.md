...
当问题只需要一个确定答案时，检索足够；但一旦任务需要决策、执行和反馈，仅仅把外部信息塞进上下文就显得力不从心。模型无法根据中间结果调整行为，也无法主动调用外部能力去改变系统状态。

从这一刻开始，大模型的角色开始发生转变：它不再只是一个“基于检索的回答器”，而需要成为一个能够思考下一步、并主动采取行动的智能体。
