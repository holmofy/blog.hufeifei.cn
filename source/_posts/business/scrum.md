...
3. **需求驳回机制**
    - **驳回条件**：
        - 逻辑矛盾（与已有功能冲突）
        - 需求描述不完整（关键流程缺失）
        - 技术不可实现（当前架构限制，研发可以先提技术改造的**研发需求**排入需求池，完成技术改造后再接产品的**用户需求**）
...
> - 需求由研发自主认领，第一轮出牌点数最少的优先领取，产品和Scrum Master只负责协调监督，不能分派，如果任务无人认领，根据故事点大小再由高级到初级依次认领。
...
1. **故事点重估**
    - 针对故事点偏差较大的需求，团队对实际完成需求重新打牌评估故事点
...
![GitFlow vs. Trunk-Based Development](https://axify.io/hs-fs/hubfs/trunk-based%20development%20vs%20gitflow%20development.webp)

| 模型                                 | 核心思路                                                    | 优点           | 缺点             |
| ---------------------------------- | ------------------------------------------------------- | ------------ | -------------- |
| **Git Flow**                       | 通过 `develop`、`feature/*`、`release/*`、`hotfix/*` 等多层分支管理 | 版本发布清晰       | 分支层级多、合并复杂、节奏慢 |
| **Trunk-Based Development (主干开发)** | 所有开发围绕 `main`（或 `trunk`）分支，每个任务小分支快速合并                  | 合并频繁，CI/CD顺畅 | 需要高测试覆盖率、CI质量  |
| **GitHub Flow**                    | 只有 `main` + 临时 `feature` 分支                             | 简洁、适合持续部署    | 无法同时维护多个版本     |
| **GitLab Flow**                    | 在环境或版本维度扩展 GitHub Flow                                  | 兼顾简单与多环境     | 对流程治理要求高       |

> 推荐采用**GitLab Flow**模型，结合CI/CD工具（如Jenkins、GitLab CI）实现自动化测试和部署，确保代码质量和交付速度。
> 
> 如果遇到两个功能需要协同开发的情况，可以开协同分支，两个功能分支基于协同分支开发，协同分支稳定后再合并回主分支。
...
* Scrum Master轮岗制度
* 前端借调制度(应对偶然出现的团队内前后端任务故事点差异过大的问题)
* 其他意见
...
|高级研发| 基础薪酬15K + 绩效奖金1K + 年终奖 + 其他(如股权激励) |

|岗位| 薪酬                            |
|---|-------------------------------|
|初级测试| 基础薪酬6K + 绩效奖金1K               |
|中级测试| 基础薪酬10K + 绩效奖金1K + 年终奖        |
|高级测试| 基础薪酬13K + 绩效奖金1K + 年终奖 + 其他(如股权激励) |

|岗位| 薪酬                                 |
|---|------------------------------------|
|初级产品| 基础薪酬10K + 绩效奖金1K                   |
|中级产品| 基础薪酬14K + 绩效奖金1K + 年终奖             |
|高级产品| 基础薪酬18K + 绩效奖金1K + 年终奖 + 其他(如股权激励) |
